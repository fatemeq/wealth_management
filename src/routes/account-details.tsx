...
const AccountDetails: React.FC = () => {
  ...
  const saveEdit = () => {
    ...
    if (!editForm.name || !editForm.currency || !editForm.icon || !editForm.color) {
      Modal.warning({ content: "All fields are required." });
      return;
    }
  ...
  const addTransaction = () => {
    if (!form.amount || Number.isNaN(Number(form.amount))) {
      Modal.warning({ content: "Please enter a valid amount." });
      return;
    }
    if (!account) return;
    ...
    const tx: Transaction = {
      id: Date.now(),
      type: form.type as "deposit" | "withdrawal",
      amount: amountNum,
    ...
    <div style={{ padding: 20 }}>
      <Row align="middle" style={{ marginBottom: 20 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          ...
          <Button danger icon={<DeleteOutlined />} onClick={deleteAccount} />
      ...
      {isEditing && (
        ...
        >
          ...
          <Input
            ...
            placeholder="Savings, Investment, Wallet…"
            ...
            {ICONS.map((emoji) => {
              ...
              return (
                <Col
                  ...
                  md={3}  // 8 icons per row on medium/large screens
                  key={emoji}
          ...
          <Row gutter={16} style={{ marginTop: 16 }}>
            <Col span={12}>
              <Button type="primary" block onClick={saveEdit}>
                Save Changes
              </Button>
            </Col>
            <Col span={12}>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        <Col xs={24} md={12}>
          <Card style={{ borderRadius: 16 }}>
            ...
            <Text type="secondary" style={{ fontSize: 12 }}>
              ≈ {baseSymbol}
              {formatMoney(balanceBase)} {baseUnit}
            </Text>
          </Card>
        </Col>
        ...
        <Col xs={24} md={12}>
          <Card style={{ borderRadius: 16 }}>
            <Text type="secondary">Profit / Loss</Text>
            <Title
              style={{
                marginTop: 8,
      ...
      <Card style={{ borderRadius: 16 }}>
        {isAdding && (
          ...
          >
            ...
            {trackPrice && (
              ...
              >
                <Row gutter={12}>
                  <Col xs={24} sm={12}>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setForm((s) => ({
                          ...s,
                          purchasePrice: formatted.replace(/,/g, ""),
                        }));
                      }}
                      style={{ marginTop: 6 }}
                    />
                  ...
                  <Col xs={24} sm={12}>
                    <Text strong>Purchase Unit</Text>
                    <Select
                      value={form.purchaseUnit || (units[0]?.code || "GBP")}
                      onChange={(v) =>
                        setForm((s) => ({ ...s, purchaseUnit: v }))
                      }
                      style={{ width: "100%", marginTop: 6 }}
                    >
                      {units.map((u) => (
            ...
            <Row gutter={16} style={{ marginTop: 24 }}>
              <Col span={12}>
                <Button type="primary" block onClick={addTransaction}>
                  Add Transaction
                </Button>
              </Col>
              <Col span={12}>
                <Button block onClick={() => setIsAdding(false)}>
                  Cancel
                </Button>
              </Col>
            </Row>
        ...
        {(account.transactions?.length ?? 0) === 0 ? (
          <div style={{ textAlign: "center", padding: 40 }}>
            ...
            <Title level={4}>No transactions yet</Title>
          </div>
        ) : (
          <div>
                ...
                <Card key={tx.id} style={{ marginBottom: 14, borderRadius: 12 }}>
                  ...
                  >
                    ...
                    <Col flex="auto" style={{ 
                      ...
                      <Text 
                        type="secondary" 
                        style={{ fontSize: 12, display: "block", whiteSpace: "nowrap", overflow: "hidden" }}
                      >
                        {tx.timestamp}
                      </Text>
